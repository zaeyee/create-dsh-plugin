...
assert.equal(root.name, 'dsh-smoke-full-mods')
  ...
  const settingsCtx = {
    settings: {
      register(namespace, schema, options) {
        assert.equal(namespace, 'dsh-smoke-full-mods')
...
// ---------- hook shape: permission gate deny/allow paths ----------
{
  ...
  assert.equal(hook.name, 'dsh-smoke-full-mods-hook')
}
...
if (existsSync(new URL('../lib/client.js', import.meta.url))) {
  ...
  assert.equal(entry.id, 'dsh-smoke-full-mods')
