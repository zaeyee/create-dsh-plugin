...
export const name = 'dsh-smoke-full-mods'
...
declare module '@deepseek-ai/cordis' {
  interface Events {
    'dsh-smoke-full-mods/ready': (payload: { id: string }) => void
  }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  installSettingsSection(ctx, settingsNamespace('dsh-smoke-full-mods'), Config, config, {
  ...
  ctx.on('dsh-smoke-full-mods/ready', ({ id }) => {
    if (configSource().verbose) console.log(`[dsh-smoke-full-mods] ready: ${id}`)
  })
}
