...
export const name = 'dsh-smoke-full'
...
declare module '@deepseek-ai/cordis' {
  interface Events {
    'dsh-smoke-full/ready': (payload: { id: string }) => void
  }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  installSettingsSection(ctx, settingsNamespace('dsh-smoke-full'), Config, config, {
  ...
  ctx.on('dsh-smoke-full/ready', ({ id }) => {
    if (configSource().verbose) console.log(`[dsh-smoke-full] ready: ${id}`)
  })
}
