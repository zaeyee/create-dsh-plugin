...
assert.equal(root.name, 'dsh-golden-shc')
  ...
  const settingsCtx = {
    settings: {
      register(namespace, schema, options) {
        assert.equal(namespace, 'dsh-golden-shc')
...
// ---------- hook shape: permission gate deny/allow paths ----------
{
  ...
  assert.equal(hook.name, 'dsh-golden-shc-hook')
}
...
if (existsSync(new URL('../lib/client.js', import.meta.url))) {
  ...
  assert.equal(entry.id, 'dsh-golden-shc')
